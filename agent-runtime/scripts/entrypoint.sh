...
# Cold Rust binaries on Northflank can take 20+ seconds to bind. Wait up to 60s.
...
# (Using `exec` would replace this shell and lose the trap, so SIGTERM from
# Northflank would not propagate to the gateway/sidecar children.)
openab run -c /tmp/openab.toml &
ab_pid=$!

# Propagate SIGTERM/SIGINT to every child so Northflank's stop signal kills
# the gateway, sidecar, and core cleanly.
