...
export async function maybeCreateSkill(userId: string, sessionId: string) {
  ...
  if (m.distinctTools.size < 2)
    return { skipped: true, reason: 'single_tool_only' };
  ...
  return withAdvisoryLock(`skill:${userId}`, async () => {
    ...
    const skillCreateTool = createTool({
      id: 'skill_create',
      description:
        'Create a new skill (SKILL.md-shaped) or patch an existing umbrella.',
    ...
    const subagent = new Agent({
      name: 'skill-creator',
      instructions: buildSkillPrompt(transcript, existing.data ?? []),
